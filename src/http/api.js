//封装所有的请求
import service from './index'

export default {
    //登录
    //usename；用户名
    //password: 密码
    // login({username,password}){
    //     return service.post('login',{usename,password})
    // },
    //1   17.获取登录注册默认验证码(get)/v1/verify
    getverifyData() {
        return service.get("/verify")
    },
    //2     23. 注册(post)  /register  参数:  nickname: 用户名  password:密码  verify:验证码
    getregister({ nickname, password, verify }) {
        return service.post('/register', { nickname, password, verify })
    },
    //3     24. 登录(post)  /login  参数:  nickname: 用户名  password:密码  verify:验证码
    getlogin({ nickname, password, verify }) {
        return service.post('/login', { nickname, password, verify })
    },
    //4     1.获取首页的数据
    getHomeData() {
        return service.get('/recommend')
    },
    //4     3. 分类查询(get)  /classification?mallSubId=${id}  参数:  id:默认分类的id
    getdataList(id) {
        return service.get(`/classification?mallSubId=${id}`)
    },
    //5     8.单个商品详情(get)  /goods/one?id=${id}  参数:  id:商品的id
    getgoodsid(id) {
        return service.get(`/goods/one?id=${id}`)
    },
    //6     14. 获取用户信息(post)  /queryUser
    getqueryUser() {
        return service.post('/queryUser')
    },
    //7     15. 修改保存用户信息(post)   /saveUser  
    //参数: gender: 性别  year: 年   month: 月  day: 日  id: 用户id  nickname: 昵称
    getsaveUser({ gender, year, month, day, id, nickname, }) {
        return service.post('/saveUser', { gender, year, month, day, id, nickname, })
    },
    //8     9. 收藏单个商品(post)   /collection  参数:   goods: 商品的详情信息
    getcollection(goods){
        return service.post('/collection',goods)
    },
    //9    10. 取消收藏(post)  /cancelCollection  参数:  id:商品的cid
    getcancelCollection(id){
        return service.post('/cancelCollection',{id})
    },
    //10   11. 查询商品是否已收藏(post)  /isCollection  参数:  id:商品的id
    getisCollection(id){
        return service.post('/isCollection',{id})
    },
    //11   12. 加入购物车(post)   /addShop  参数:  id:商品的id
    getaddshop(id){
        return service.post('/addShop',{id})
    },
    //12   4. 查询获取购物车数据(post)  /getCard
    getCard(){
        return service.post('/getCard')
    },
    //13   6. 购物车商品删除(post)   /deleteShop  参数:   商品id(数组)
    getdeleteShop(id){
        return service.post('/deleteShop',id)
    },
    //14   5. 购物车加减商品(post)   /editCart   参数: count: 购物车数量    id: 商品id   mallPrice: 价格
    geteditCart({count,id,mallPrice}){
        return service.post('/editCart',{count,id,mallPrice})
    },
    //15    21. 增加收货地址(post)   /address  
    //参数:  name:用户名  tel:电话  address:(省+市+区+详情地址)  isDefault:是否默认 
    //province:省  city:市  county:区  addressDetail:详情地址，  areaCode:地区代码  id:修改地址时候要传id
    getaddress({name,tel,address,isDefault,province,city,county,addressDetail,areaCode,id}){
        return service.post('/address',{name,tel,address,isDefault,province,city,county,addressDetail,areaCode,id})
    },
    //16    18. 查询用户收货地址(get)  /getAddress
    getgetAddress(){
        return service.get('/getAddress')
    },
    //17    22. 删除地址(post)  /deleteAddress  参数:  id:地址_id
    getdeleteAddress(id){
        return service.post('/deleteAddress',{id})
    },
    //18    20. 设置默认收货地址(post)   /setDefaultAddress  参数:  id: 地址id
    getsetDefaultAddress(id){
        return service.post('/setDefaultAddress',{id})
    },
    //19    19. 查询默认收货地址(get)  /getDefaultAddress
    getgetDefaultAddress(){
        return service.get('/getDefaultAddress')
    },
    //20    7. 购物车支付页面(post)  /order  参数:  address:收货地址  tel:电话  orderId:所有商品的id(数组)  
    //totalPrice:总价格  idDirect:用来判断是购物车结算还是直接购买(直接购买为true,购物车结算为false)  count:商品数量
    getorder({address,tel,orderId,totalPrice,idDirect,count}){
        return service.post('/order',{address,tel,orderId,totalPrice,idDirect,count})
    },
    //21    29.订单查询路径是 myOrder
    getmyOrder(){
        return service.get('/myOrder')
    },
    //22    27. 查询未评价(get)  /tobeEvaluated
    gettobeEvaluated(){
        return service.get('/tobeEvaluated')
    },
    //24    16. 商品评论(post)  /goodsOne/comment   参数:  id: 商品cid  rate: 分数  content: 
    //内容  anonymous: 是否匿名  _id: 商品的_id  order_id: 商品的order_id  image: []
    getcomment({id,rate,content,anonymous,_id,order_id,image}){
        return service.post('/goodsOne/comment',{id,rate,content,anonymous,_id,order_id,image})
    },
    //25    26. 查询已评价(get)  /alreadyEvaluated
    getalreadyEvaluated(){
        return service.get('/alreadyEvaluated')
    },
    //26     28. 查询单条评论(post)  /evaluateOne参数: id:商品id  _id:数据库的那条id
    getevaluateOne({id,_id}){
        return service.post('/evaluateOne',{id,_id})
    },
    //27     2. 搜索(post)  /search  参数: value: 搜索关键词
    getsearch({value}){
        return service.post('/search',{value})
    },
    //28     30. 查询我的收藏(get)   
    getshoucang(){
        return service.get('/collection/list')
    },
    //13. 退出登录(post)	/loginOut
    getloginout(){
        return service.post('/loginOut')
    }
}